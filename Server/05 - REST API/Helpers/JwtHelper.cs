...
namespace Tomedia
{
    public class JwtHelper
    {
        ...
        public string GetJwtToken(string username, string role, string id)
        {
            // Create JWT Claims: 
            Claim claimByUsername = new Claim(ClaimTypes.Name, username);
            Claim claimByRole = new Claim(ClaimTypes.Role, role);
            Claim claimByBusinessID = new Claim("businessId", id);
            List<Claim> claims = new List<Claim> { claimByUsername, claimByRole, claimByBusinessID };
            ...
            securityTokenDescriptor.Expires = DateTime.UtcNow.AddHours(24);
